...
def show_predict_page():
    ...
    plt.ylabel('TotalEnergy(kJ)', fontsize=18)
    ...
    # Calculation
    pred_view = st.slider('Enter the number of views', min_value=0, max_value=15000000, value=7500000, step=10000)
    pred_energy = calculation(pred_view)
    st.markdown(f"""Estimated energy consume for the selected view is : {pred_energy:.0f} kJ""") 
show_predict_page()
